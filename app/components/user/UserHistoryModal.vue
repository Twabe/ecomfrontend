...
// Use the service
const { getByUserId, getActionLabel, parseMetadata } = useOrderHistoriesService()

// Create a computed ref for userId
const userIdRef = computed(() => props.userId ?? undefined)

// Query for history by user ID
const { data: history, isLoading, error } = getByUserId(userIdRef)
          ...
          >
            <DialogPanel class="w-full max-w-3xl transform rounded-xl bg-white p-6 shadow-xl dark:bg-gray-800">
              ...
              <DialogTitle class="flex items-center gap-2 text-lg font-semibold text-gray-900 dark:text-white">
                ...
                Historique d'activite - {{ userName }}
              </DialogTitle>

              <!-- Stats Cards -->
              <div v-if="history && history.length > 0" class="mt-4 grid grid-cols-2 md:grid-cols-4 gap-3">
                <div class="rounded-lg bg-gray-50 dark:bg-gray-700/50 p-3 text-center">
                  <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ stats.total }}</p>
                  <p class="text-xs text-gray-500 dark:text-gray-400">Total Actions</p>
                </div>
                <div class="rounded-lg bg-blue-50 dark:bg-blue-900/20 p-3 text-center">
                  <p class="text-2xl font-bold text-blue-600 dark:text-blue-400">{{ stats.stateChanges }}</p>
                  <p class="text-xs text-gray-500 dark:text-gray-400">Changements d'etat</p>
                </div>
                <div class="rounded-lg bg-green-50 dark:bg-green-900/20 p-3 text-center">
                  <p class="text-2xl font-bold text-green-600 dark:text-green-400">{{ stats.confirmations }}</p>
                  <p class="text-xs text-gray-500 dark:text-gray-400">Confirmations</p>
                </div>
                <div class="rounded-lg bg-indigo-50 dark:bg-indigo-900/20 p-3 text-center">
                  <p class="text-2xl font-bold text-indigo-600 dark:text-indigo-400">{{ stats.suivi }}</p>
                  <p class="text-xs text-gray-500 dark:text-gray-400">Suivi</p>
                </div>
              </div>
              ...
              <div class="mt-4 max-h-[60vh] overflow-y-auto">
                ...
                <div v-else-if="!history || history.length === 0" class="py-8 text-center">
                  ...
                  <p class="mt-2 text-gray-500 dark:text-gray-400">Aucune activite pour cet utilisateur</p>
                </div>
                  ...
                  >
                    ...
                    <div class="flex gap-3">
                      ...
                      <div class="flex-1 -mt-0.5 min-w-0">
                        <!-- Header row: Action type + Order Code + Date -->
                        <div class="flex items-center justify-between gap-2 flex-wrap">
                          <div class="flex items-center gap-2 flex-wrap">
                            ...
                            <!-- Order Code (clickable in future) -->
                            <span
                              v-if="item.orderId"
                            ...
                            <span
                              ...
                              class="inline-flex rounded-full bg-indigo-100 px-2 py-0.5 text-xs font-medium text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-300"
                            >
