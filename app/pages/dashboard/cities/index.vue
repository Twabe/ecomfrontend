<template>
  <div>
            ...
            >
              <DialogPanel class="w-full max-w-md bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6">
                <DialogTitle class="text-lg font-semibold text-gray-900 dark:text-white mb-4">
                  {{ editingCity ? $t('common.edit') : $t('common.create') }} {{ $t('nav.cities') }}
                </DialogTitle>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $t('nav.cities') }}</h1>
        <p class="text-gray-600 dark:text-gray-400 mt-1">{{ pagination.totalCount }} {{ $t('nav.cities') }}</p>
      </div>
      ...
      <div v-else class="overflow-x-auto">
        <table class="min-w-full">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="city in cities" :key="city.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
              <td class="py-3 px-4">
...
import { PlusIcon, MagnifyingGlassIcon, PencilIcon, TrashIcon, MapPinIcon } from '@heroicons/vue/24/outline'
import { useCitiesService, type CityDto } from '~/services'

definePageMeta({
  layout: 'tenant',
  middleware: ['auth', 'tenant', 'permission'],
  requiredPermission: 'Permissions.Cities.View'
})
...
const openEditModal = (city: CityDto) => {
...
const handleDelete = async (city: CityDto) => {
