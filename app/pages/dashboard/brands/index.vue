<template>
  <div>
            ...
            >
              <DialogPanel class="w-full max-w-md bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6">
                <DialogTitle class="text-lg font-semibold text-gray-900 dark:text-white mb-4">
                  {{ editingBrand ? $t('common.edit') : $t('common.create') }} {{ $t('nav.brands') }}
                </DialogTitle>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $t('nav.brands') }}</h1>
        <p class="text-gray-600 dark:text-gray-400 mt-1">{{ pagination.totalCount }} {{ $t('nav.brands') }}</p>
      </div>
        ...
        <table class="min-w-full">
          <thead>
            <tr class="border-b dark:border-gray-700">
              ...
              <th class="text-left py-3 px-4 text-sm font-medium text-gray-500 dark:text-gray-400">{{ $t('products.description') }}</th>
              <th class="text-right py-3 px-4 text-sm font-medium text-gray-500 dark:text-gray-400">{{ $t('common.actions') }}</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="brand in brands" :key="brand.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
              <td class="py-3 px-4">
                <span class="font-medium text-gray-900 dark:text-white">{{ brand.name }}</span>
              </td>
              <td class="py-3 px-4">
                <span class="text-gray-600 dark:text-gray-400 text-sm">{{ brand.description || '-' }}</span>
              </td>
...
import { useBrandsService, type BrandDto } from '~/services'

definePageMeta({
  layout: 'tenant',
  middleware: ['auth', 'tenant', 'permission'],
  requiredPermission: 'Permissions.Brands.View'
})
...
const openEditModal = (brand: BrandDto) => {
  ...
  form.value = { name: brand.name || '', description: brand.description || '' }
  showModal.value = true
}
...
const handleSubmit = async () => {
  ...
  try {
    if (editingBrand.value) {
      await update(editingBrand.value.id!, {
