<template>
  <div>
            ...
            >
              <DialogPanel class="w-full max-w-md bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6">
                <DialogTitle class="text-lg font-semibold text-gray-900 dark:text-white mb-4">
                  {{ editingExtraTag ? $t('common.edit') : $t('common.create') }} {{ $t('nav.extraTags') }}
                </DialogTitle>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $t('nav.extraTags') }}</h1>
        <p class="text-gray-600 dark:text-gray-400 mt-1">{{ pagination.totalCount }} {{ $t('nav.extraTags') }}</p>
      </div>
    ...
    <div class="card">
      ...
      <div v-else-if="extraTags.length === 0" class="text-center py-12">
        <TagIcon class="w-12 h-12 text-gray-400 mx-auto mb-3" />
      ...
      <div v-else class="overflow-x-auto">
        <table class="min-w-full">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="extraTag in extraTags" :key="extraTag.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
              <td class="py-3 px-4">
                <span class="font-medium text-gray-900 dark:text-white">{{ extraTag.title }}</span>
              </td>
              <td class="py-3 px-4 text-right">
                <div class="flex items-center justify-end gap-2">
                  <button
                    @click="openEditModal(extraTag)"
...
import { useExtraTagsService, type ExtraTagDto } from '~/services'

definePageMeta({
  layout: 'tenant',
  middleware: ['auth', 'tenant', 'permission'],
  requiredPermission: 'Permissions.ExtraTags.View'
})
...
const editingExtraTag = ref<ExtraTagDto | null>(null)
...
const openCreateModal = () => {
  editingExtraTag.value = null
  form.value = { title: '' }
  showModal.value = true
}

const openEditModal = (extraTag: ExtraTagDto) => {
  editingExtraTag.value = extraTag
  form.value = { title: extraTag.title || '' }
  showModal.value = true
}
...
const handleSubmit = async () => {
  ...
  try {
    if (editingExtraTag.value) {
      await update(editingExtraTag.value.id!, {
        id: editingExtraTag.value.id!,
...
const handleDelete = async (extraTag: ExtraTagDto) => {
  if (confirm(t('messages.confirmDelete'))) {
    try {
      await remove(extraTag.id!)
