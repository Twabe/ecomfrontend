<template>
  <div>
            ...
            >
              <DialogPanel class="w-full max-w-md bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6">
                <DialogTitle class="text-lg font-semibold text-gray-900 dark:text-white mb-4">
                  {{ editingReason ? $t('common.edit') : $t('common.create') }} {{ $t('nav.reasons') }}
                </DialogTitle>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $t('nav.reasons') }}</h1>
        <p class="text-gray-600 dark:text-gray-400 mt-1">{{ pagination.totalCount }} {{ $t('nav.reasons') }}</p>
      </div>
        ...
        <table class="min-w-full">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="reason in reasons" :key="reason.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
              <td class="py-3 px-4">
                <span class="font-medium text-gray-900 dark:text-white">{{ reason.title }}</span>
              </td>
...
import { PlusIcon, MagnifyingGlassIcon, PencilIcon, TrashIcon, ExclamationTriangleIcon } from '@heroicons/vue/24/outline'
import { useReasonsService, type ReasonDto } from '~/services'

definePageMeta({
  layout: 'tenant',
  middleware: ['auth', 'tenant', 'permission'],
  requiredPermission: 'Permissions.Reasons.View'
})
...
const editingReason = ref<ReasonDto | null>(null)
...
const openEditModal = (reason: ReasonDto) => {
  editingReason.value = reason
  form.value = { title: reason.title || '' }
  showModal.value = true
}
...
const handleSubmit = async () => {
  ...
  try {
    if (editingReason.value) {
      await update(editingReason.value.id!, {
        id: editingReason.value.id!,
...
const handleDelete = async (reason: ReasonDto) => {
