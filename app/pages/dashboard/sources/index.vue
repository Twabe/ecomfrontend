<template>
  <div>
            ...
            >
              <DialogPanel class="w-full max-w-md bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6">
                <DialogTitle class="text-lg font-semibold text-gray-900 dark:text-white mb-4">
                  {{ editingSource ? $t('common.edit') : $t('common.create') }} {{ $t('nav.sources') }}
                </DialogTitle>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $t('nav.sources') }}</h1>
        <p class="text-gray-600 dark:text-gray-400 mt-1">{{ pagination.totalCount }} {{ $t('nav.sources') }}</p>
      </div>
      ...
      <div v-else class="overflow-x-auto">
        <table class="min-w-full">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="source in sources" :key="source.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
              <td class="py-3 px-4">
                <span class="font-medium text-gray-900 dark:text-white">{{ source.title }}</span>
              </td>
...
import { PlusIcon, MagnifyingGlassIcon, PencilIcon, TrashIcon, MegaphoneIcon } from '@heroicons/vue/24/outline'
import { useSourcesService, type SourceDto } from '~/services'

definePageMeta({
  layout: 'tenant',
  middleware: ['auth', 'tenant', 'permission'],
  requiredPermission: 'Permissions.Sources.View'
})
...
const editingSource = ref<SourceDto | null>(null)
...
const openEditModal = (source: SourceDto) => {
...
const handleSubmit = async () => {
  ...
  try {
    if (editingSource.value) {
      await update(editingSource.value.id!, {
...
const handleDelete = async (source: SourceDto) => {
